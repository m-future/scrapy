...
class jbk39(scrapy.Spider):  # 需要继承scrapy.Spider类
    ...
    def start_requests(self):
        ...
        diseases = db.select_disease('fuke')

        # print(diseases)


        for disease in diseases:
            keyword={'wd':disease['name']}
            encode= urllib.parse.urlencode(keyword)
            url='{}?{}'.format(base_url, encode)
            print(url)
        ...
        for department in departments[1:2]:
            ...
            url = '{}?{}'.format(base_url, department["pinyin"])
